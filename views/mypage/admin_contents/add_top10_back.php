...
<?php if(count($result) > 0):?>
			<form method='post' action='<?php echo site_url('admin_contents/add_top10') ?>'>
			<table class='list'>
			<tr><th>画像</th><th>広告名</th><th>商品コード</th><th>商品名</th><th>価格</th><th>並び順</th><th></th><th></th><th></th></tr>
	<?php foreach($result as $row):?>
			<tr>
		<?php if($row->product_code):?><?php // 商品マスタからの登録 ?>
			<?php if($row->p_image_name):?>
				<td><img src='<?php echo base_url() . PRODUCT_IMAGE_PATH . $row->p_image_name ?>' width='60' height='60'></td>
			<?php else:?>
				<td><img src='<?php echo base_url('images/no-image.jpg') ?>' width='60' height='60'></td>
			<?php endif; ?>
				<td>-</td>
		...
		<?php endif; ?>
		<?php if($row->advertise_id):?><?php //チラシからの登録 ?>
			<?php if($row->ad_pro_image_name): ?>
				<td><img src='<?php echo base_url() . AD_PRODUCT_IMAGE_PATH . $row->ad_pro_image_name ?>' width='60' height='60'></td>
			<?php else: ?>
				<td></td>
			<?php endif; ?>
			...
			<form action='' method='post'>
				<table class='detail' cellpadding='0' cellspacing='10'>
					...
					<tr>
						...
						<td><input type='submit' name='submit' value='登録する'><a class='edit_back' href='<?php echo site_url('admin_contents/add_top10') ?>'>戻る</a></td>
					</tr>
				</table>
			</form>
		</div>
...
function del_confirm(template_name , id){
	...
	if(window.confirm(template_name + '削除してもよろしいですか')){
		location.href='<?php echo site_url("/admin_contents/delete_top10") ?>' + '/' + id;
		return false;
	}
}
$('select[name=show_flag]').change(function(){
			...
			location.href='<?php echo site_url('admin_contents/change_show_flag_top10') ?>' + '/' + str_id + '/' + data ;
});
</script>
</html>