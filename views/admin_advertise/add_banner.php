...
<?php $end_date = new DateTime($row->end_datetime) ?>
					...
					<td><a class='edit' href='<?php echo site_url("/admin_advertise/edit_banner/{$row->id}") ?>'>変更</a></td>
...
//$('#end_date').datepicker('setDate','9999/12/31');

function del_confirm(template_name , id){
	var template_name = template_name;
	var id = id;
	if(window.confirm(template_name + 'を削除してもよろしいですか')){
		location.href='<?php echo site_url("/admin_advertise/delete_banner") ?>' + '/' + id;
		return false;
	}
}
$('select[name=show_flag]').change(function(){
			...
			location.href='<?php echo site_url('admin_advertise/change_show_flag_banner') ?>' + '/' + str_id + '/' + data ;
});
</script>
</body>
</html>