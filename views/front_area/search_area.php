		...
		<h2><span class='logo_pink'>area</span> <?php echo $h2title ?></h2>
			...
			<?php if(isset($is_area)):?>
					...
					<tr>
						...
						<td><span>宅配スーパー配達可能エリアです</span><br>会員登録していただくことで配送料0円～で配送いたします。</td>
						<td><?php echo $result->prefecture ?> <?php echo $result->city ?><?php echo $result->address ?></td>
					</tr>
					<tr>
						<td></td>
						<td><p><a class='button' href='<?php echo base_url('front_customer/show_policy')?>'>会員登録を進める（会員規約表示）</a></p></td>
					</tr>
					<?php else: ?>
					<tr>
						<th class='result'><label>判定</label></th>
						<td><span>宅配便でご希望の商品を発送致します。</td>
					</tr>
					<tr>
						<td></td>
						<td><p><a class='button' href='<?php echo base_url('front_customer/show_policy')?>'>会員登録を進める（会員規約表示)</a></p></td>
			...
			<h3>住所から検索</h3>
				<div id='map'>
					...
					<map name='map'>
						<area href ='<?php echo base_url('front_area/show_area/1')?>' shape='rect' alt='金沢地区' coords='50,420,200,480'>
						<area href ='<?php echo base_url('front_area/show_area/2')?>' shape='rect' alt='能登地区' coords='100,210,250,270'>
					</map>
				</div>
		</div>
	</div>
