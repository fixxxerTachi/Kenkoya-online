	...
	<div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
		...
		<main class="mdl-layout__content mdl-color--grey-100">
			<div class="mdl-grid demo-content">
				<div class="container">
					<div class='contents'>
						<h2><span class='logo_pink'>mainvisual</span> メインビジュアル登録リスト</h2>
							...
							<tr><th>画像</th><th>サムネイル</th><th>掲載開始日時</th><th>掲載終了日時</th><th>リンク先</th><th>掲載順</th><th></th><th></th><th></th></tr>
							<?php foreach($result as $row):?>
								<?php $s_date = new DateTime($row->start_datetime) ?>
								<?php $e_date = new DateTime($row->end_datetime)?>
								<tr>
									<td>
										<a href='<?php echo base_url() . MAINVISUAL_IMAGE_PATH . $row->image_name ?>' id='image_modal' data-lightbox='<?php echo $row->image_name ?>'><img src='<?php echo base_url() . MAINVISUAL_IMAGE_PATH . $row->image_name ?>' width='100' height='38'></a><br>
										<?php echo $row->image_name ?>
									</td>
									<td>
								<?php if(!empty($row->thumb_image_name)):?>
										<img src='<?php echo base_url(MAINVISUAL_IMAGE_PATH . $row->thumb_image_name) ?>' width='100' height='38'><br>
										<?php echo $row->thumb_image_name ?>
								<?php endif;?>
									</td>
									<td><?php echo $s_date->format('Y/m/d H:i');?></td>
									<td><?php echo $e_date->format('Y/m/d H:i');?></td>
							...
							<?php endforeach;?>
								<tr class='no-back'><td></td><td></td><td></td><td></td><td></td><td><input type='submit' name='change_order' value='掲載順変更'></td><td></td><td></td><td></td></tr>
							</table>
							</form>
							<?php else:?>
							<p>登録されていません</p>
					...
					<div class='contents'>
							<h2><span class='logo_pink'>mainvisual</span> <?php echo $h2title ?></h2>
							<?php if(!empty($success_message)):?>
							<p class='success'><?php echo $success_message; ?></p>
							<?php endif; ?>
							<?php if(!empty($error_message)):?>
							...
							<?php echo form_open_multipart() ?>
								<table class='detail' cellpadding='0' cellspacing='10'>
									...
									<tr>
										...
										<td>
											<input type='text' name='image_name' id='image_name' value='<?php echo $form_data->image_name ?>'>
											<?php if(!empty($form_data->image_name)):?>
											<img src='<?php echo base_url() . MAINVISUAL_IMAGE_PATH . $form_data->image_name ?>' width='100' height='38'>
											<?php endif;?>
										</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td>
											<input type='text' id='description' name='description' value='<?php echo $form_data->description ?>' size='70' maxlength='70'>
										</td>
									</tr>
									<tr>
