	...
	public function add_advertise()
	{
		...
		if($this->input->post('submit')){
			...
			if($this->form_validation->run() === FALSE){
				...
				redirect('/admin_advertise/add_advertise');
			}
		}
		...
		if(isset($_FILES['csvfile'])){
			try{
				...
				foreach($csv as $row){
					...
					$stmt->bindValue(':calorie',$row[11]);
					$stmt->bindValue(':note',$row[12]);
				...
				redirect(site_url('/admin_advertise/add_advertise'));
			}catch(Exception $e){
...
//ここまで	
	...
	public function list_product()
	{
		...
		if($this->session->userdata('params')){
			$form_data = $this->session->userdata('params');
		...
		$config['per_page'] = 20;
		$config['base_url'] = base_url("admin_advertise/list_product/{$advertise_id}/");
		if($this->input->post('search')){
			...
			$form_data = (object)$form_data;
			$this->session->set_userdata('params',$form_data);
			$offset = 0;
			$result = $this->Advertise->get_products_by_advertise_id_with_image($advertise_id,$form_data);
			$this->data['result'] = $result;
			$this->session->set_userdata('total_rows',count($result));
				...
				if(!empty($_FILES['image']['name'])){
					if(is_uploaded_file($_FILES['image']['tmp_name'])){
						if(move_uploaded_file($_FILES['image']['tmp_name'],'images/products/' . $_FILES['image']['name'])){
							...
							redirect(site_url('admin_advertise/list_product/' . $ad_id));
...
//				$image_info = pathinfo($_FILES['image']['name']);
				...
				if(is_uploaded_file($_FILES['image']['tmp_name'])){
					if(move_uploaded_file($_FILES['image']['tmp_name'],$image_path.$_FILES['image']['name'])){
						...
						$this->session->set_flashdata('success','登録しました');
						redirect(site_url('/admin_advertise/upload_image/' . $ad_id));
					}else{
						unlink($_FILES['image']['tmp_name']);
						$this->data['error_message'] = 'ファイルのアップロードに失敗しました';
					}
