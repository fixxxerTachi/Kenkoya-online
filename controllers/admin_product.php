...
//ここから		
		if(isset($_FILES['csvfile'])){
			try{
				...
				$error_arr=array();
				foreach($csv as $row){
					$stmt->bindValue(':category_code',$row[4]);
					$stmt->bindValue(':category_name',$row[5]);
					$stmt->bindValue(':vendor_code',$row[1]);
					$stmt->bindValue(':vendor_name',$row[2]);
					$stmt->bindValue(':product_code',$row[9]);
					$stmt->bindValue(':branch_code',$row[10]);
					$stmt->bindValue(':kana',$row[13]);
					$stmt->bindValue(':product_name',$row[12]);
					$stmt->bindValue(':short_name',$row[14]);
					$stmt->bindValue(':order_sort',$row[11]);
					$stmt->bindValue(':sale_price',$row[16]);
					$stmt->bindValue(':cost_price',$row[17]);
					$stmt->bindValue(':adddate',$row[18]);
					$stmt->bindValue(':moddate',$row[19]);
				...
				redirect(base_url('/admin_product/upload'));
			}catch(Exception $e){
				...
				if(!empty($_FILES['image']['name'])){
					...
					if(is_uploaded_file($_FILES['image']['tmp_name'])){
						if(move_uploaded_file($_FILES['image']['tmp_name'],'images/products/' . $_FILES['image']['name'])){
							$result = $this->Product->resize(PRODUCT_IMAGE_PATH.$_FILES['image']['name'],$db_data->product_code,300);
							if($result){
								//$db_data->image_name = $_FILES['image']['name'];
								$db_data->image_name = 'ak' . $db_data->product_code . '.jpg';
				...
				if(is_uploaded_file($_FILES['image']['tmp_name'])){
					if(move_uploaded_file($_FILES['image']['tmp_name'],'images/products/' . $_FILES['image']['name'])){
						...
						if($result){
							...
							//$this->load->database();
							//$this->db->trans_begin();
							$this->Product->db->trans_begin();
							$result=$this->Product->update($id,$db_data);
							$allergen = $this->input->post('allergen');
							$this->Product->update_allergen($id,$pa_middle_ids,$allergen);
							if($this->Product->db->trans_status === FALSE){
								$this->Product->db->trans_rollback();
							}else{
								$this->Product->db->trans_commit();
								$this->session->set_flashdata('success','登録しました');
//								if($old_image){ unlink($old_image);};
								redirect(base_url('admin_product/list_product'));
							}
						}else{
							$this->data['error_message'] = 'ファイルのアップロードに失敗しました';
						}
...
*/	
	...
	{
		//アレルゲンリスト
		$this->data['list_result'] = $this->Allergen->show_list();
				...
				//ファイルがアップロードされた場合
					if(is_uploaded_file($_FILES['icon']['tmp_name'])){
						...
						//ファイルのアップロードに成功した場合
							...
							$this->session->set_flashdata('success','登録しました');
							redirect(base_url('/admin_product/add_allergen'));
						}else{
						//アップロードに失敗した場合
	...
	public function product_charge()
	{
		...
		if($this->input->post('search'))
		{
			...
			{
				$result = $this->Product_size->get_by_product_code($form_data->product_code);
				if(empty($result)){
					$this->session->set_flashdata( '登録されていません' );
					return redirect('admin_product/product_charge');
				}
				$form_data = $result;
				$update_flag = True;
			}else{
		...
		if($this->input->post('update'))
		{
			...
			if(!empty($form_data->product_code))
			{
				...
				$this->Product_size->update_by_code($form_data->product_code,$db_data);
				$this->session->set_flashdata('success','更新しました');
				return redirect('admin_product/product_charge');
			}else{
				$this->data['error_message'] = '入力していない項目があります';
			}
		}
		//submitは商品登録
		if($this->input->post('submit'))
		{
			$form_data = (object)$this->input->post();
			if(!empty($form_data->product_code) || !empty($weight))
			{
				$db_data = array(
					'product_code'=>$form_data->product_code,
					'temp_zone_id'=>(int)$form_data->temp_zone,
				...
				$this->Product_size->save($db_data);
				$this->session->set_flashdata('success','登録しました');
				return redirect('admin_product/product_charge');
			}else{
				return redirect('admin_product/product_charge');
		...
		if(isset($_FILES['csvfile']))
		{
			try
			{
				...
				$upload_file_message = $upload_file_info['message'];
				$csv = convertCsvToDb($upload_file_name);
				$this->data['upload_file_message'] = $upload_file_message;
				$dbh = getDb();
				if($this->input->post('trancate'))
				{
				...
				*/
				...
				foreach($csv as $row)
				{
					...
					$stmt->bindValue(':depth',$row[5]);
					$stmt->bindValue(':volume',$row[6]);
					$stmt->execute();
					$error_info = $stmt->errorInfo();
					$error_arr[] = $stmt->errorInfo();
					if($error_info[0] != '00000')
				...
				$this->session->set_flashdata('success',"{$counter}件データベースに登録しました");
				return redirect(base_url('admin_product/product_charge'));
			}
			catch(Exception $e)
			{
				$this->data['error_message'] = $e->getMessage();
...
/*
		...
		{
			try{
				//codeを取得するためにAdvertise_productモデルを取得
				$this->load->model('Advertise_product');
				$ad_id = $this->input->post('ad_id');
				if(empty($ad_id))
				{
					throw new Exception('対象となる台帳を選択してください');
				}
				$upload_file_info = uploadCsv('csvfile2');
				$upload_file_name = $upload_file_info['uploaded_file_name'];
				$upload_file_message = $upload_file_info['message'];
				$csv = convertCsvToDb($upload_file_name);
				$this->data['upload_file_mesasge'] = $upload_file_message;
				...
				foreach($csv as $row)
				{
					...
					if($error_info[0] != '00000')
					{
						$this->data['error_message'] = 'データベースに登録しました';
						break;
					}
				}
				$counter = 0;
...
*/
		...
		{
			try{
				$ad_id = $this->input->post('ad_id_no_size');
				if(empty($ad_id)){
		...
		//どのチラシの商品台帳から取り込むか選択するためチラシリストを取得
		$this->load->model('Advertise');
		$this->data['ad_list'] = $this->Advertise->show_list_arr_no_limit();
		$this->data['update_flag'] = $update_flag;
		$this->data['form_data'] = $form_data;
		$this->data['list_temp_zone'] = $this->Takuhai_charge->list_temp_zone();
		$this->data['success_message']  = $this->session->flashdata('success');
		$this->load->view('admin_product/product_charge',$this->data);	
	}
