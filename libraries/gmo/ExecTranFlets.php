...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranFletsInput();/* @var $input ExecTranFletsInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new ExecTranFlets();/* @var $exec ExecTranFlets */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output ExecTranFletsOutput */
...
//ExecTranFlets入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranFlets.php' );