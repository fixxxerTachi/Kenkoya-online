		...
		<div class='content'>
			...
			<?php echo form_open() ?>
				<table class='contact_form' cellpadding='0' cellspacing='10'>
					<p class='note'>宅配スーバー健康屋オンライン会員の方はこちらからユーザー名、パスワードを入力してログインしてください </p>
...
<?php if(empty($user_view)):?>
			...
			<?php echo form_open('front_customer/login_action') ?>
				<table class='member_form'>
					<p class='note'>会員登録を希望される方は「会員登録する」、会員登録しないで購入を希望される方は、「会員登録しない」をチェックして「次へ」クリックしてください。</p>
