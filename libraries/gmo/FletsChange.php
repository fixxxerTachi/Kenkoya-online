...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/FletsChangeInput.php');
	require_once( 'com/gmo_pg/client/tran/FletsChange.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new FletsChangeInput();/* @var $input FletsChangeInput */
	...
	$input->setChangeAmount($_POST['ChangeAmount']);
	$input->setChangeTax($_POST['ChangeTax']);

	
	//API通信クラスをインスタンス化します
	$exe = new FletsChange();/* @var $exec FletsChange */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output FletsChangeOutput */
...
//FletsChange入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/FletsChange.php' );